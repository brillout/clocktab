...
//assert this.style.display=='inline-block'
{
  ...
  */
  ...
  else if(possibleChars) //if possibleChars not given we then assume that all char have same width
  {
    ...
    assert.warning(possibleChars.constructor === Array && possibleChars.length>0);
  ...
  //******** Refinment ********
  assert.warning(this.getStyle('display')==='block' || this.getStyle('display')==='inline-block' || this.getStyle('display')==='table-cell',"this.getStyle('display')=="+this.getStyle('display'),1);
  this.hide();
  //==old approx
